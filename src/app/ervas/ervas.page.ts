...
@Component({
  selector: 'app-ervas',
  templateUrl: './ervas.page.html',
  styleUrls: ['./ervas.page.scss'],
})
export class ErvasPage {
  ...
  constructor(public navCtrl: NavController,private platform: Platform, public alertCtrl: AlertController,
    ...
    this.storage.get('produtos').then(async res => {
      this.storage.get('lojas').then(data =>{
          ...
          this.goalList = res.filter(i => i.estado === this.estado && i.aprovado === 'Sim' && i.tipoPrd ==='Ervas')
